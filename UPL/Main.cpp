...
#define NAMEVERSION "UP v1-alpha"
...
bool IsUPFunction(const string& funcName)
{
	return startsWith(funcName, "UP.");
}
...
boost::any EvalExpression(const string& ex, unordered_map<string, boost::any>& variableValues)
{
	...
	bool isUP = split(expression, '.')[0] == "UP";
	// If no operations are applied, then return self
	if ((countOutsideParenthesis(expression, '+') == 0 && countOutsideParenthesis(expression, '-') == 0 && countOutsideParenthesis(expression, '*') == 0 && countOutsideParenthesis(expression, '/') == 0 && countOutsideParenthesis(expression, '^') == 0) || split(expression, '.')[0] == "UP")
	{
		//bool isFunc = IsFunction(split(expression, '(')[0]);
		if (isFunc && !inQuotes)
		{
...
#endif
			vector<boost::any> funcArgs = VarValues(argList, variableValues);
			return UPFunction(split(expression, '(')[0], funcArgs);
		}
		else
	...
	for (int i = 0; i < expression.size(); i++)
	{
		...
		if (isalpha(expression[i]) || expression[i] == '_')
		{
			...
			else if (split(name, '.')[0] == "UP" && !inQuotes)
			{
				// start -> FuncCall(0, x, OtherFunc(a))
				// changeto -> 0, x, OtherFunc(a)
				string insideFunArgs = betweenChars(expression, '(', ')');
...
#endif
				vector<boost::any> funcArgs = VarValues(argList, variableValues);
				string returnVal = AnyAsString(UPFunction(split(expression, '(')[0], funcArgs));
				newExpression += returnVal;
			}
			else
			{
...
boost::any ProcessLine(const vector<vector<string>>& words, int& lineNum, unordered_map<string, boost::any>& variableValues)
{
	...
	// If print statement (deprecated, now use UP.System.Print() function)
	...
	// Check if it is UP Builtin function call
	else if (startsWith(words.at(lineNum).at(0), "UP."))
...
int main(int argc, char* argv[])
{
	...
	if (argc > 1 || EXAMPLE_PROJECT)
	{
		...
		if (EXAMPLE_PROJECT)
			scriptPath = "D:\\Code\\UPL\\examples\\Platformer\\script.UP";
		else
			scriptPath = argv[1];
#if DEVELOPER_MESSAGES
	...
	{ // If no script is provided as an argument throw error
		LogWarning("No script provided! Please drag and drop .up file over interpreter executable file, or provide it's path as a command-line argument.");
