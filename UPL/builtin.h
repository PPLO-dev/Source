...
int InterpreterLog(const string& logText)
{
	...
	PrintColored("UP-LANG: ", yellowFGColor, "", true);
	PrintColored(escaped(logText), greenFGColor, "", true);
	cout << std::endl;
	//cout << "\x1B[34m[" + to_string(Hour) + ":" + to_string(Min) + ":" + to_string(Sec) + "] \x1B[33mUP: \x1B[32m" << logText << "\033[0m\t\t" << endl;
	return 1;
}
...
int LogCriticalError(const string& errorText)
{
	...
	cin.ignore();
	exit(1);
	//cerr << "\x1B[34m[" + to_string(Hour) + ":" + to_string(Min) + ":" + to_string(Sec) + "] \x1B[33mUP: \x1B[31mERROR: " << errorText << "\033[0m\t\t" << endl;
	//exit(EXIT_FAILURE);
	return 2;
}
...
boost::any UPFunction(const string& name, const vector<boost::any>& args)
{
	...
	else if (name == "UP.Graphics.Sprite")
	{
	...
	else if (name == "UP.Graphics.DrawPixel")
	{
	...
	else if (name == "UP.Graphics.Draw")
		any_cast<Sprite>(args.at(0)).Draw();
	else if (name == "UP.Graphics.Load")
		any_cast<Sprite>(args.at(0)).Load();
	else if (name == "UP.Graphics.Text")
	{
	...
	else if (name == "UP.System.Vec2")
	{
		...
		LogWarning("UP function \'" + name + "\' does not exist.");
